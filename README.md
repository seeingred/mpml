...
All twelve players use real screenshots with live SVG overlays fitted to their native coordinates. This version began on `experiment/real-screenshots` and is now published from `main`. The generated-image version remains available in Git history at `6ef9cee`.
...
The first screen offers “Choose a song” and “Use a YouTube link” at the same size, in that order, followed by a subtle “or play demo” option. The demo starts in one click. Play reveals the player, subtle caption and year timeline. Twelve landmarks are spaced evenly over the song; seeking moves the music, crossfade, and window geometry together. Dragging the timeline pauses audio until release and the final seek finishes, then resumes only if it was playing before. Keyboard seeking follows the same behavior. Pausing freezes the transition and updates the embedded playback indicator. Replay returns to Winamp 2. Clicking the screenshot toggles playback, including replay after the song ends. The screenshot also supports Enter and Space when focused. Space toggles playback elsewhere; reduced motion shows the nearest player without blending.
...
The year labels are preserved: 1998, 2002, 2003, 2004, 2006, 2008, 2010, 2011, 2015, 2019, 2020, 2026. Screenshot dates may differ. See [SCREENSHOT-SOURCES.md](SCREENSHOT-SOURCES.md) for every original source, version limitation, attribution, and license note.

## Images and overlays

- `players.js` defines each original image, native dimensions, SVG crop, stage bounds, and source details.
- `assets/real/` holds twelve original screenshot files. Desktop backgrounds are cropped in SVG; downloaded files remain unchanged.
